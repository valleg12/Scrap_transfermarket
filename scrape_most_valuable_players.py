        ...
        for page in range(1, total_pages + 1):
            ...
            for player in players:
                try:
                    ...
                    nationality_img = columns[4].select_one('img')
                    nationality = nationality_img['title'] if nationality_img else "Unknown"
                    
                    
                    club_element = columns[5].select_one('img.tiny_wappen')
                    ...
                    if name and position and age:
                        ...
                        competition_stats = self.get_detailed_stats(player_url)
                        player_data['competition_stats'] = competition_stats
                    ...
                    print(f"Error while retrieving stats for a player: {str(e)}")
                    continue
...
def main():
    scraper = MostValuablePlayersScraper()
    base_url = 'https://www.transfermarkt.com/spieler-statistik/wertvollstespieler/marktwertetop?land_id=0&ausrichtung=alle&spielerposition_id=alle&altersklasse=alle&jahrgang=0&kontinent_id=0&plus=1'
    ...
    df.to_csv('most_valuable_players_stats.csv', index=False, encoding='utf-8')
    print(f"Stats saved in most_valuable_players_stats.csv ({len(df)} players)")
    
    
    df.to_json('most_valuable_players_stats.json', orient='records', force_ascii=False, indent=4)
    print(f"Stats saved in most_valuable_players_stats.json ({len(df)} players)")
